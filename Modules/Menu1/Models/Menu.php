...
class Menu extends BaseModel
{
    ...
    {
        // CACHE DISABLED: return Cache::rememberForever("menu_version_{$location}", function () {
        //     return (string) time();
        // });
        return (string) time();
    }
    ...
    public static function getCachedMenuData(string $location, $user = null, ?string $locale = null): \Illuminate\Support\Collection
    {
        ...
        // CACHE DISABLED: $version = static::getMenuCacheVersion($location);

        // CACHE DISABLED: Create cache key based on location, user id, locale, AND version
        // CACHE DISABLED: $userId = $user ? $user->id : 'guest';
        // CACHE DISABLED: $cacheKey = "menu_data_{$location}_{$userId}_{$locale}_{$version}";

        // CACHE DISABLED: return Cache::remember($cacheKey, 3600, function () use ($location, $user, $locale, $defaultLocale) {
        return (function () use ($location, $user, $locale, $defaultLocale) {
    ...
    {
        // CACHE DISABLED: Caching is disabled, so no cache clearing is needed
        // if ($location) {
    ...
    {
        // CACHE DISABLED: Caching is disabled
        // Cache::flush();
    }
