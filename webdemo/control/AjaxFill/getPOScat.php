...
where tc.status='Active' and tc.app_reject is null order by tc.p_cat_id DESC $LimitQry");
    ...
    case 2:#Approved links
		...
		$rs=$obj->simplefetch("select tc.pos_cat_id,tc.pos_cat_name,concat(wu.user_name,' [',date_format(tc.entry_date,'%d/%m/%Y'),' ]')as cName from pos_cat tc 
INNER JOIN web_users wu on wu.user_id=tc.entry_by
where tc.status='Active' order by tc.pos_cat_id DESC $LimitQry");
		
        if($rs[0]>0){
            $sNo=(!empty($_REQUEST['start']))?intval($_REQUEST['start']):'0';    
            foreach($rs[1] as $row){                
                $data[] = array(++$sNo,
                    html_entity_decode($row['pos_cat_name']),
                    $row['cName'],  
                    "<div class=\"text-center tools\">
                        <i class=\"fa fa-edit\" cdata-frmT='2' data-pos_cat_id='$row[pos_cat_id]' title='Modify category'></i>
                        <i class=\"fa fa-trash-o\" cdata-frmT='3' data-pos_cat_id='$row[pos_cat_id]' title='Delete category'></i>
                    </div>",
                );
            }
        }
