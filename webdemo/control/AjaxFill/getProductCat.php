...
where tc.status='Active' and tc.app_reject is null order by tc.p_cat_id DESC $LimitQry");
    ...
    case 2:#Approved links
        $rs=$obj->simplefetch("select tc.p_cat_id,tc.cat_name,concat(wu.user_name,' [',date_format(tc.creation_date,'%d/%m/%Y'),' ]')as cName,concat(ru.user_name,' [',date_format(tc.app_rej_action_on,'%M %d, %Y'),' ]')as apName from web_product_category tc 
INNER JOIN web_users wu on wu.user_id=tc.creator_id
INNER JOIN web_users ru on ru.user_id=tc.app_rej_user_id
where tc.status='Active' and tc.app_reject=1 order by tc.p_cat_id DESC $LimitQry");
        if($rs[0]>0){
            $sNo=(!empty($_REQUEST['start']))?intval($_REQUEST['start']):'0';    
            foreach($rs[1] as $row){                
                $data[] = array(++$sNo,
                    ...
                    $row['apName'],     
                    "<div class=\"text-center tools\">
                        <i class=\"fa fa-edit\" cdata-frmT='2' data-p_cat_id='$row[p_cat_id]' title='Modify category'></i>
                        <i class=\"fa fa-trash-o\" cdata-frmT='3' data-p_cat_id='$row[p_cat_id]' title='Delete category'></i>
                    </div>",
                );
            }
        }
