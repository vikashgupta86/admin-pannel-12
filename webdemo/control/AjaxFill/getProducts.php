...
where pt.status='Active' and mc.p_cat_id=$_REQUEST[p_cat_id] and pt.publish_by is null order by pt.p_temp_id DESC $LimitQry");
    ...
    case 2:#Published links
        $rs=$obj->simplefetch("SELECT mt.product_name,mf.pid,mt.price,mt.discount,mt.bdesc,mt.p_temp_id,concat(wu.user_name,' [',date_format(mt.creation_date,'%d/%m/%Y'),' ]')as cName/*,ifnull(concat(ru1.user_name,' [',date_format(mt.app_rej_action_on,'%M %d, %Y'),' ]'),'N/A')as apName*/,concat(pu.user_name,' [',date_format(mf.publish_date,'%M %d, %Y'),' ]')as pName from web_product_temp mt
INNER JOIN web_product_category mc on mc.p_cat_id=mt.p_cat_id
INNER JOIN web_product_final mf on mf.p_temp_id=mt.p_temp_id
...
where mt.status='Active' and mc.p_cat_id=$_REQUEST[p_cat_id] and mf.status='Active' and mt.app_reject=1 order by mt.p_temp_id DESC $LimitQry");
        if($rs[0]>0){
            $sNo=(!empty($_REQUEST['start']))?intval($_REQUEST['start']):'0';    
            foreach($rs[1] as $row){                
                $data[] = array(++$sNo,
                    ...
                    "<div class=\"text-center tools\">
                        <i class=\"fa fa-edit\" cdata-frmT='2' pub-pid='$row[pid]' data-p_temp_id='$row[p_temp_id]' title='Modify Details'></i>
                        <i class=\"fa fa-trash-o\" cdata-frmT='3' pub-pid='$row[pid]' data-p_temp_id='$row[p_temp_id]' title='Delete Product'></i>
						<i class=\"fa fa-shopping-bag\" cdata-frmT='4' pub-pid='$row[pid]' data-p_temp_id='$row[p_temp_id]' title='Add Packaging'></i>
                    </div>",
                );
            }
        }
