...
$rs= simplefetch("select tt.t_id,tt.t_temp_id,tt.tender_name,concat(wu.user_name,' [',date_format(tt.creation_date,'%Y-%m-%d'),' ]')as cName,concat(au.user_name,' [',date_format(tt.app_rej_action_on,'%d/%m/%Y'),' ]')as aName from web_tender_temp tt
        ...
        LEFT JOIN (select * from web_tender_final where status='Active') tf on tf.t_temp_id=tt.t_temp_id
         where tt.`status`='Active' and tt.t_cat_id= ".$_REQUEST['t_cat_id']." and tt.corrigendum=1 and tt.publish_by is null and tt.publish_by is null and tt.app_reject=1 $LimitQry");
        if($rs[0]>0){
            $sNo=(!empty($_REQUEST['start']))?intval($_REQUEST['start']):'0';    
            foreach($rs[1] as $row){  
		$tname=getNameQry("select concat('[',tt1.t_num,']',tt1.tender_name) as tname from web_tender_temp tt1 
			INNER JOIN web_tender_final tf2 on tf2.t_temp_id=tt1.t_temp_id 
			where tf2.t_id=$row[t_id] ");	
				...
				$tname,
                    ...
                    "<div class=\"text-center tools\">
                        <i class=\"fas fa-desktop\" cdata-frmT='1' ten-t_id=".$row['t_id']." data-t_temp_id=".$row['t_temp_id']." title='Publish'></i>
                    </div>",
                );
            }
        }
