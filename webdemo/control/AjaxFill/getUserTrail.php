...
switch($_REQUEST['tabID']){
    case 1:#Login attemps
        ...
        $rs=$obj->simplefetch("select case when wu.user_id is null then 'Unknown' else concat(wup.f_name,'<br>( ',wu.uname,' )') end as lUser,date_format(wl.dateoflogin,'%M %d, %Y %T')as dateoflogin,ifnull(date_format(wl.dateoflogoff,'%M %d, %Y %T'),'---')as dateoflogoff,wl.ipaddress,wl.system_ip,wl.browser from web_loginoroffusertrail wl
LEFT JOIN web_users wu on wu.user_id=wl.userid
INNER JOIN web_users_profile wup on wup.user_id=wu.user_id
 $subQry order by wl.id DESC $LimitQry");
        if($rs[0]>0){
    ...
    case 2:#Unsuccessful attempt
        ...
        $rs=$obj->simplefetch("select case when wu.user_id is null then 'Unknown' else concat(wup.f_name,'<br>( ',wu.uname,' )') end as lUser,date_format(wa.dated,'%M %d, %Y %T')as dated,wa.ipaddress,wa.system_ip,wa.browser from web_attempts wa
LEFT JOIN web_users wu on wu.user_id=wa.pid 
INNER JOIN web_users_profile wup on wup.user_id=wu.user_id
$subQry order by wa.sno DESC $LimitQry");
        if($rs[0]>0){
            $sNo=(!empty($_REQUEST['start']))?intval($_REQUEST['start']):'0';    
            foreach($rs[1] as $row){                
                $data[] = array(++$sNo,                                        
    ...
    case 3:#user trail
        ...
        $rs=$obj->simplefetch("select ut.actiontaken,ut.tablename,ut.system_ip,case when wu.user_id is null then 'Unknown' else concat(wup.f_name,'<br>( ',wu.uname,' )') end as lUser,concat(date_format(ut.dated,'%M %d, %Y'),' ', ut.timed)as dated,ut.ip_addr,ut.browser from web_user_trail ut
LEFT JOIN web_users wu on wu.user_id=ut.user_id 
INNER JOIN web_users_profile wup on wup.user_id=wu.user_id
$subQry order by ut.id DESC $LimitQry");
