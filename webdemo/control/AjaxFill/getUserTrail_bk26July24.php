...
switch($_REQUEST['tabID']){
    case 1:#Login attemps
        ...
        $rs=$obj->simplefetch("select case when wu.user_id is null then 'Unknown' else concat(wu.user_name,'<br>( ',wu.uname,' )') end as lUser,date_format(wl.dateoflogin,'%M %d, %Y %T')as dateoflogin,ifnull(date_format(wl.dateoflogoff,'%M %d, %Y %T'),'---')as dateoflogoff,wl.ipaddress,wl.system_ip,wl.browser from web_loginoroffusertrail wl
LEFT JOIN web_users wu on wu.user_id=wl.userid $subQry order by wl.id DESC $LimitQry");
    ...
    case 2:#Unsuccessful attempt
        ...
        $rs=$obj->simplefetch("select case when wu.user_id is null then 'Unknown' else concat(wu.user_name,'<br>( ',wu.uname,' )') end as lUser,date_format(wa.dated,'%M %d, %Y %T')as dated,wa.ipaddress,wa.system_ip,wa.browser from web_attempts wa
LEFT JOIN web_users wu on wu.user_id=wa.pid $subQry order by wa.sno DESC $LimitQry");
    ...
    case 3:#user trail
        ...
        $rs=$obj->simplefetch("select ut.actiontaken,ut.tablename,ut.system_ip,case when wu.user_id is null then 'Unknown' else concat(wu.user_name,'<br>( ',wu.uname,' )') end as lUser,concat(date_format(ut.dated,'%M %d, %Y'),' ', ut.timed)as dated,ut.ip_addr,ut.browser from web_user_trail ut
