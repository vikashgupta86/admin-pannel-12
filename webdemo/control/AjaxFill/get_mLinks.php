...
$dept_type_id = (isset($_REQUEST['dept_type_id']) && $_REQUEST['dept_type_id'] != '-1') ? (int)$_REQUEST['dept_type_id'] : 0;
if ($dept_type_id === 0 && isset($_REQUEST['nmnh_type']) && $_REQUEST['nmnh_type'] != '-1') {
    $dept_type_id = (int)$_REQUEST['nmnh_type'];
}
...
where wlt.`status`='Active' and lf.`status`='Active' and wlt.continuous_content=0 and wlt.lang_id = ".$lang_id." AND wlt.content_type = ". $dept_type_id ." $subQry order by ls.pos_id, TempPos+0, wlt.link_name",1);
...
// where wlt.`status`='Active' and lf.`status`='Active' and wlt.continuous_content=0 and wlt.lang_id=$_REQUEST[lang_id] $subQry order by ls.pos_id, TempPos+0, wlt.link_name $LimitQry",1);
    ...
    case '2':
        ...
        INNER JOIN web_link_type lt on lt.type_id=wlt.type_id
        LEFT JOIN (select * from web_links_structure where status='Active' and link_level is null) ls on ls.lid=lf.lid
        where wlt.`status`='Active' and lf.`status`='Active' and wlt.continuous_content=0 and wlt.lang_id = ".$lang_id." AND wlt.content_type = ".$dept_type_id ." $subQry order by ls.pos_id DESC, TempPos+0, wlt.link_name",1);
                if($rs[0]>0){
                    $sNo=(!empty($_REQUEST['start']))?intval($_REQUEST['start']):'0';    
                    foreach($rs[1] as $row){                        
                        $dClass='disabled';
    ...
    default:
        ...
        /*INNER JOIN web_link_type lt on lt.type_id=wlt.type_id*/
        LEFT JOIN (select * from web_links_structure where status='Active' and link_level is null) ls on ls.lid=lf.lid
        where wlt.`status`='Active' and lf.`status`='Active' and wlt.continuous_content=0 and wlt.lang_id= ".$lang_id." AND wlt.content_type = ".$dept_type_id ." $subQry $usr_session order by ls.pos_id DESC, TempPos+0, wlt.link_name",1);
                ...
                if($rs[0]>0){
                    ...
                    foreach($rs[1] as $row){
                        ...
                        $data[] = array(++$sNo,
                            ...
                            "<div class=\"text-center\">
                                <button type=\"button\" class=\"btn btn-sm btn-success single-row-update\" data-id=\"$row[lid]\"><i class=\"fa fa-save\"></i></button>
                            </div>"
                        );
