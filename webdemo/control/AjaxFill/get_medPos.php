...
$rs=simplefetch("SELECT mt.image_name,case when mf.gallery_flage=1 then 'checked=\"\"' else '' end as bChk,mf.m_id,mt.m_description,mt.m_description_h,mt.m_temp_id,mt.m_name,mt.m_name_h,mt.type_of_media,case when mt.type_of_media=1 then 'Photo' else 'Video' end as mType,concat(wu.user_name,' [',date_format(mt.creation_date,'%d/%m/%Y'),' ]')as cName,ifnull(concat(ru1.user_name,' [',date_format(mt.app_rej_action_on,'%M %d, %Y'),' ]'),'N/A')as apName,concat(pu.user_name,' [',date_format(mf.publish_date,'%M %d, %Y'),' ]')as pName,mf.pos from web_media_temp mt
        ...
        if($rs[0]>0){
            ...
            foreach($rs[1] as $row){
                ...
                $data[] = array(++$sNo,
                    ...
                    $cImage,
                    $row['apName'],     
                    "<div class=\"text-center\"><input type=\"number\" class=\"form-control\" style=\"width:65px;\" name=\"pos_$row[m_id]\" id=\"pos_$row[m_id]\" value=\"$row[pos]\" data-validate=\"pos_$row[m_id]|text|n|1|10|num|Please enter Valid Number!\"  />
                    <input type='hidden'  name=\"mChk[]\"  value=\"$row[m_id]\"></div>",
                );
            }
        }
