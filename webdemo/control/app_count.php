...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['type1']) && $_POST['frmType'] == 1) {
        ...
        $suc = $obj->insert("app_count", $Fields, $Values);
        if ($suc) {
            $subsec_id = $suc[0];
        ...
        $rs1 = $obj->simplefetch("select * from app_count where app_count_id='" . $_POST['app_count_id'] . "' and status='Active'",1);
        if ($rs1[0] > 0) {
            ...
            $suc = $obj->update("app_count", $Fields, $Values, "app_count_id='" . $_POST['app_count_id'] . "'",1);
            if ($suc) {
        ...
        <div class="content-wrapper">
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">Application received Count</h3>
                </div>
                ...
                <div class="box-body clearfix">
                    ...
                    <div class="panel panel-default">
                        <ul class="nav nav-tabs" id="myTab">
                            <li class="active"><a tab-ind='1' data-target="#notification" data-toggle="tab">Year wise Application Count</a></li>
                        </ul>
                                ...
                                <table id="myTable1" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                    <thead>
                                        <tr>
                                            <th>S.No</th>
                                            <th>Year</th>
                                            <th>No of received Applications</th>
...
<script type="text/javascript">
    ...
    $(document).delegate('#AddNew,.fa-edit,.fa-trash-o,.fa-retweet,.fa-desktop', 'click', function() {
        ...
        if ($.inArray(frmType, ['3', '4', '5', '6']) != '-1') {
            ...
            var eleCtrl = $(this);
            var app_count_id = ($(this).attr('data-app_count_id'));
            ...
            if (con) {
                ...
                $.ajax({
                    ...
                    dataType: "text",
                    cache: false,
                    url: '<?php echo "del_appCount.php?per_id=$_SESSION[per_id]&EncHid=$_SESSION[EncTok]"; ?>',
                    data: {
            ...
            var lid = ($(this).attr('data-app_count_id') != undefined) ? $(this).attr('data-app_count_id') : '';
            $('.modal-container').OpenPop({
                url: "add_appCount.php?frmType=" + frmType + "&per_id=<?php echo "$_SESSION[per_id]&EncHid=$_SESSION[EncTok]" ?>&app_count_id=" + lid,
            });
        } else {
            $('.modal-container').OpenPop({
                url: "add_appCount.php?frmType=" + frmType + "&per_id=<?php echo "$_SESSION[per_id]&EncHid=$_SESSION[EncTok]"; ?>",
            });
        }
    });
        ...
        if ($('#myTab').length > 0) {
            ...
            $('#myTab a').each(function(i, d) {
                ...
                tempTable = $('table#myTable' + tabID).DataTable({
                    ...
                    "serverSide": false,
                    "ajax": {
                        "url": "AjaxFill/appCountList.php?<?php echo "EncHid=$_SESSION[EncTok]"; ?>",
                        "type": "POST",
                        "data": {
                            <?php echo $GLOBALS['csrf']['token']; ?>,
                            'tabID': tabID,
