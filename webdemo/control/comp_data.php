...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['type1']) && $_POST['frmType'] == 1) {
        ...
        // $rs1 = $obj->simplefetch("select * from comp_data where year='" . $_POST['comp_data_id'] . "' and status='Active'",1);

        $year = $_POST['year_id'];
        
        //$state_id = $_POST['state_id'];
        $sector_id = $_POST['sector_id'];
        //die ("year=".$year."--".$sector_id);
        $subsector_id = $_POST['subsector_id'];
        $first = $_POST['first'];
        $second = $_POST['second'];
        $cert_merit = $_POST['cert_merit'];
        ...
        $FielArr = array(
            ...
            'first' => $first,
            'second' => $second,
            'cert_merit' => $cert_merit,
        );

        $Fields = implode(',', array_keys($FielArr));
        $Values = implode("|$$|", $FielArr);
        $suc = $obj->insert("comp_data", $Fields, $Values);
        if ($suc) {
            $subsec_id = $suc[0];
        ...
        $rs1 = $obj->simplefetch("select * from comp_data where comp_data_id='" . $_POST['comp_data_id'] . "' and status='Active'",1);
        if ($rs1[0] > 0) {
            ...
            $suc = $obj->update("comp_data", $Fields, $Values, "comp_data_id='" . $_POST['comp_data_id'] . "'",1);
            if ($suc) {
        ...
        <div class="content-wrapper">
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">Competition Data</h3>
                </div>



                <div class="box-body clearfix">
                    <div class="text-right" style="margin-bottom: 2px;"><button id="AddNew" cdata-frmT='1' class="btn btn-warning">Add New Data</button></div>
                    <div class="panel panel-default">
                        <ul class="nav nav-tabs" id="myTab">
                            <li class="active"><a tab-ind='1' data-target="#notification" data-toggle="tab">Competition Data</a></li>
                        </ul>
                                ...
                                <table id="myTable1" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center">Certificate of Merit</th>
...
<script type="text/javascript">
    ...
    $(document).delegate('#AddNew,.fa-edit,.fa-trash-o,.fa-retweet,.fa-desktop', 'click', function() {
        ...
        if ($.inArray(frmType, ['3', '4', '5', '6']) != '-1') {
            ...
            var eleCtrl = $(this);
            var comp_data_id = ($(this).attr('data-comp_data_id'));
            ...
            if (con) {
                ...
                $.ajax({
                    ...
                    dataType: "text",
                    cache: false,
                    url: '<?php echo "del_compData.php?per_id=$_SESSION[per_id]&EncHid=$_SESSION[EncTok]"; ?>',
                    data: {
            ...
            var lid = ($(this).attr('data-comp_data_id') != undefined) ? $(this).attr('data-comp_data_id') : '';
            $('.modal-container').OpenPop({
                url: "add_compData.php?frmType=" + frmType + "&per_id=<?php echo "$_SESSION[per_id]&EncHid=$_SESSION[EncTok]" ?>&comp_data_id=" + lid,
            });
        } else {
            $('.modal-container').OpenPop({
                url: "add_compData.php?frmType=" + frmType + "&per_id=<?php echo "$_SESSION[per_id]&EncHid=$_SESSION[EncTok]"; ?>",
            });
        }
    });
    ...
    $(function() {
        if ($('#myTab').length > 0) {
            ...
            $('#myTab a').each(function(i, d) {
                ...
                console.log(tabID + 'hi')
                tempTable = $('table#myTable' + tabID).DataTable({
                    "processing": true,
                    "serverSide": false,
                    "ajax": {
                        "url": "AjaxFill/compDataList.php?<?php echo "EncHid=$_SESSION[EncTok]"; ?>",
                        "type": "POST",
                        "data": {
                            <?php echo $GLOBALS['csrf']['token']; ?>,
                            'tabID': tabID,
