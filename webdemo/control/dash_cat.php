            ...
            <input type="hidden" name="prog_id" id="prog_id" value="<?php echo $_REQUEST['prog_id'];?>" />
                  ...
                  <div class="row">
                    <div class="col-12 mb-3">
                      ...
                      <input type="text" class="form-control" name="c_name" id="c_name" placeholder="Category Name" value="<?php echo $row['prog_name'] ?? '';?>" data-validate="c_name|text|y|1|500|alnum_spcA|Please enter Valid Category Name!" required/>
                    </div>
                    
                    <div class="col-12 mb-3">
                      <label for="hc_name" class="form-label">Category Name (Hindi) <span class="text-danger">*</span></label>
                      <input type="text" class="form-control" name="hc_name" id="hc_name" placeholder="Category Name in Hindi" value="<?= $row['prog_name_hin'] ?? '';?>" data-validate="hc_name|text|n|1|5000|alnum_spcA|Please enter Valid Category Name!" required/>
                    </div>      
                    
                    <div class="mb-3">
					    <label for="dash_sb_link_id" class="form-label">Sub Links</label>
                        <select name="dash_sb_link_id" id="dash_sb_link_id" class="form-control" data-validate="dash_sb_link_id|text|y|1|10|num|dontselect=-1|Please enter Valid option!" >
                            <option value="">--- Select ---</option>
                            <?php
                                $sql = "SELECT lt.link_temp_id,lt.link_name,lf.lid FROM web_links_structure ls INNER JOIN web_links_final lf ON ls.lid = lf.lid INNER JOIN web_link_temp lt ON lt.link_temp_id = lf.link_temp_id where ls.parent_ls_id = 60 and ls.status = 'Active' ORDER BY lt.link_name ASC";
                                ...
                                foreach($allprgData as $row1)
                                {
                                    if(($row['dash_sb_link_id'] ?? '')==$row1['link_temp_id'])
                                        echo "<option value='$row1[lid]' selected=''>$row1[link_name]</option>";
                                    else
                                        echo "<option value='$row1[lid]'>$row1[link_name]</option>";
                                }
