...
?>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <div class="user-panel d-flex align-items-center">
            <img src="assets/images/avatar.png" class="rounded-circle me-3" alt="User Image" width="45" height="45">
            <div>
                <p class="text-white text-wrap">Welcome <?php echo $prow['sName'];?></p>
                <a href="javascript:void(0)" class="text-decoration-none text-white small"><i class="bi bi-circle-fill text-success" style="font-size: 10px;"></i> Online</a>
            </div>
        </div>
        ...
        <nav class="sidebar-nav">
          <div class="nav flex-column" id="sidebarNavAccordion">
                ...
                <a href="<?php echo base_url()."/control/main.php?EncHid=$_SESSION[EncTok]";?>" class="nav-link"><i class="fas fa-tachometer-alt"></i> Dashboard</a>
     ...
     $rsqury = "SELECT DISTINCT
    m.module_id,
    m.module_name,
    m.abbrev,
    IFNULL(m.fa_icon,'fa-arrow-right') AS fa_icon,
    m.pos
FROM web_user_permission up
JOIN web_st_sub_module sm 
    ON sm.sub_module_id = up.sub_module_id AND sm.status='Active'
JOIN web_st_module m 
...
ORDER BY m.pos;";
              ...
              if($rsmodule){
                foreach ($rsmodule as $rowmodule){
                  ...
                  $rssbqury = "SELECT ifnull(sm.fa_icon,'fa-circle-o') as s_fa_icon,sm.pos,up.per_id,sm.sub_module_id,sm.sub_module_page,sm.sub_module_name FROM web_user_permission up,web_st_module m, web_st_sub_module sm WHERE up.status='Active' AND m.status='Active' AND sm.status='Active' AND m.module_id=sm.module_id AND sm.sub_module_id=up.sub_module_id AND up.user_id = ? AND m.module_id = ? 
                  AND sub_module_name NOT IN ('Manage Continuous Content', 'Set Continuous Link Postion')
                  order by sm.pos";
                  $rssbconn = db_connect();
                  ...
                  //if($rowmodule['module_name'] != 'Tender' && $rowmodule['module_name'] != 'Feedback' && $rowmodule['module_name'] != 'Others')
                  //{
                  ?>

                      <a href="javascript:void(0)" data-bs-toggle="collapse" data-bs-target="#block_<?php echo $rowmodule['module_id'];?>" aria-expanded="false" class="nav-link d-flex justify-content-between align-items-center">
                        <span>
                      ...
                      <?php //}
                        $rowmodule_module_id = "block_" . $rowmodule['module_id'];
                        if($rssubmodule){
                          echo '<ul class="collapse sub-menu" id="' . $rowmodule_module_id . '" data-bs-parent="#sidebarNavAccordion">';
  ...
  $(function(){
    ...
    ?>
    // Correctly handle Bootstrap 5 collapse and active states
    var $targetMenu = $('#<?php echo $mID;?>');
    if ($targetMenu.length) {
        $targetMenu.addClass('show');
        $('[data-bs-target="#<?php echo $mID;?>"]').attr('aria-expanded', 'true').addClass('active');
    }
    <?php
    }
    ...
    ?>    
    $('#<?php echo $smID;?>').find('a').addClass('active-sub');
    <?php
    }
    ?>
