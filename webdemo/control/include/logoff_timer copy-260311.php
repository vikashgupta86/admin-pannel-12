...
<script type="text/javascript">
  ...
  function start_countdown(){
   myVar= setInterval(function(){ 
    if(counter>=0){      
     document.getElementById("countdown").innerHTML="You Will Be Logged Out In <span class='text-bold'>"+SecondsTohhmmss(counter)+" Sec </span> <span id='ses_rest' class='btn btn-default btn-xs'>Reset Session</span>";
   }
 ...
 $(function(){
  $(document).delegate("#ses_rest", "click", function() {   
    ...
    $('*').bind('mousemove click mouseup mousedown keydown keypress keyup submit change mouseenter scroll resize dblclick', function () {
      ...
      if(idleState == false){        
        $("#ses_rest").trigger('click');
      }

        $("body").css('background-color','#000');
        idleState = true; }, 5000);//five second
    });
})
