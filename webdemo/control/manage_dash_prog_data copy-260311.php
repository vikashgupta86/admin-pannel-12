            ...
            if($.inArray(frmType,['3','4','5','6'])!='-1'){
                ...
                if(con){                        
                    ...
                    $.ajax({
                        ...
                        data     : {<?php echo $GLOBALS['csrf']['token'];?>,'frmType':frmType,'prog_data_id':$(this).data('prog_data_id')},
                        beforeSend: function(){
                            $.fn.ajaxLoading();
                        },        
                ...
                $('.modal-container').OpenPop({
                    url:"dash_prog.php?frmType=" + frmType +"&prog_data_id=" + $(this).data('prog_data_id') + "&lang_id=<?php // echo "$_REQUEST[lang_id]";?>&type_id=<?= $_REQUEST['type_id'] ?? '';?>&per_id=<?php echo "$_SESSION[per_id]&EncHid=$_SESSION[EncTok]";?>",
                    title: "Add/ Modify Details"
                });
            }
        });
            ...
            if($('#myTab').length>0)
            {
                ...
                $('#myTab a').each(function(i,d){
                    ...
                    tempTable=$('table#myTable'+tabID).DataTable( {
                        ...
                        "serverSide": false,
                        "ajax": {
                            "url":"AjaxFill/getDashProgData.php?<?php echo "EncHid=$_SESSION[EncTok]";?>",
                            "type":"POST",
