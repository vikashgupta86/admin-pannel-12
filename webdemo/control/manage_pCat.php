...
<h2 class="h5 content-header text-dark">Manage Photo Category</h2>
    ...
    <button id="AddNew" cdata-frmT="1" class="btn btn-warning rounded-0 fw-bold px-3 py-1 add-btn"><i class="fas fa-plus-square me-1" aria-hidden="true"></i>Add New Link</button>
                    ...
                    <div class="panel panel-default">                        
                        <ul class="nav nav-tabs border-bottom-0" id="myTab" role="tablist">
                            <li class="nav-item"><button class="nav-link rounded-0 text-dark border border-bottom-0 active" data-bs-toggle="tab" data-bs-target="#pending" type="button">Pending Categories</button></li>
                            <li class="nav-item"><button class="nav-link rounded-0 text-dark border border-bottom-0" data-bs-toggle="tab" data-bs-target="#publish" type="button">Published Categories</button></li>
                            <li class="nav-item"><button class="nav-link rounded-0 text-dark border border-bottom-0" data-bs-toggle="tab" data-bs-target="#rejected" type="button">Rejected Categories</button></li>
                        </ul>
        ...
        $(document).delegate('#AddNew,.fa-edit,.fa-trash-o,.fa-eye,.fa-eye-slash,.fa-retweet', 'click', function() {
            ...
            if($.inArray(frmType,['3','4','5','6'])!='-1'){
                ...
                if(con){                        
                    ...
                    $.ajax({
                        ...
                        dataType: "text",
                        cache    : false,
                        url      : '<?php echo "photo_cat_action.php?per_id=$_SESSION[per_id]&EncHid=$_SESSION[EncTok]";?>',
            ...
            {      
                $('.modal-container').OpenPop({
                    url:"photo_cat.php?frmType=" + frmType +
                    "&m_cat_id=" + $(this).data('m_cat_id') + 
                    "&lang_id=<?php echo "$_REQUEST[lang_id]";?>
                    &type_id=<?= $_REQUEST['type_id'] ?? '';?>&per_id=<?php echo "$_SESSION[per_id]&EncHid=$_SESSION[EncTok]";?>",
            ...
            if($('#myTab').length){
                ...
                $('#myTab button').each(function(index){
                    ...
                    const table = $('#myTable'+tabID).DataTable({
                        ...
                        serverSide: false,
                        ajax: {
                            url: "AjaxFill/getPhotoCat.php?<?= "EncHid=$_SESSION[EncTok]"; ?>",
                            type: "POST",
                            data: {
                                <?= $GLOBALS['csrf']['token']; ?>,
                                tabID: tabID,
