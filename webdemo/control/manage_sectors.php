...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['type1']) && $_POST['frmType'] == 1) {
        ...
        $suc = $obj->insert("neca_sectors", $Fields, $Values);
        if ($suc) {
            $sec_id = $suc[0];

            echo "<script>alert('Sector Created Successfully');</script>";
        }
        
    } else if (isset($_POST['type1']) && $_POST['frmType'] == 9) {

        $rs1 = $obj->simplefetch("select * from neca_sectors where sector_id='" . $_POST['sec_id'] . "' and status='Active'");
        if ($rs1[0] > 0) {
            ...
            $suc = $obj->update("neca_sectors", $Fields, $Values, "sector_id='" . $_POST['sec_id'] . "'");
            if ($suc) {
        ...
        <div class="content-wrapper">
            <div class="box">
                ...
                <div class="box-body clearfix">
                    <div class="text-right" style="margin-bottom: 2px;"><button id="AddNew" cdata-frmT='1' class="btn btn-warning">Add New Sectors</button></div>
                    <div class="panel panel-default">
                        <ul class="nav nav-tabs" id="myTab">
                            <li class="active"><a tab-ind='1' data-target="#notification" data-toggle="tab">Sectors</a></li>
                        </ul>
...
<script type="text/javascript">
    ...
    $(document).delegate('#AddNew,.fa-edit,.fa-trash-o,.fa-retweet,.fa-desktop', 'click', function() {
        ...
        if ($.inArray(frmType, ['3', '4', '5', '6']) != '-1') {
            ...
            var eleCtrl = $(this);
            var sec_id = ($(this).attr('data-sector_id'));
            ...
            if (con) {
                ...
                $.ajax({
                    ...
                    dataType: "text",
                    cache: false,
                    url: '<?php echo "del_sector.php?per_id=$_SESSION[per_id]&EncHid=$_SESSION[EncTok]"; ?>',
                    data: {
            ...
            var lid = ($(this).attr('data-sector_id') != undefined) ? $(this).attr('data-sector_id') : '';
            $('.modal-container').OpenPop({
                url: "add_sector.php?frmType=" + frmType + "&per_id=<?php echo "$_SESSION[per_id]&EncHid=$_SESSION[EncTok]" ?>&sec_id=" + lid,
            });
        } else {
            $('.modal-container').OpenPop({
                url: "add_sector.php?frmType=" + frmType + "&per_id=<?php echo "$_SESSION[per_id]&EncHid=$_SESSION[EncTok]"; ?>",
            });
        }
    });
        ...
        if ($('#myTab').length > 0) {
            ...
            $('#myTab a').each(function(i, d) {
                ...
                tempTable = $('table#myTable' + tabID).DataTable({
                    ...
                    "serverSide": false,
                    "ajax": {
                        "url": "AjaxFill/sectorsList.php?<?php echo "EncHid=$_SESSION[EncTok]"; ?>",
                        "type": "POST",
                        "data": {
                            <?php echo $GLOBALS['csrf']['token']; ?>,
                            'tabID': tabID,
