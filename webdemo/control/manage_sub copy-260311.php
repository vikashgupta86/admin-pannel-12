...
?>
          ...
          <!-- Left side column. contains the logo and sidebar -->
                  ...
                  <h3 class="box-title">Manage Sub Links</h3>
                ...
                if(isset($_REQUEST['preSub']) && $_REQUEST['preSub']==1 && ($frmError ?? '')==false){
                    ...
                    $ShowGridFlage=true;
                ?>    
                     <div class="parent-hir clearfix">       
                <div class="box-body ">                    
                    <div class="panel panel-default">                        
                        <h4 class="">Set/ Unset Sublinks<?php if(isset($_SESSION['musume_type'])){ echo $_SESSION['musume_type']; } ?></h4>
                        ...
                        if($_SESSION['user_type'] =='20')
                        {
                            ...
                            where ls.`status`='Active' and ls.link_level is null and lt.lang_id=$lang_id $condition ORDER BY lt.link_name",1);
                        ...
                        {
							$rs = simplefetch("select nls.tot,ls.ls_id,ifnull(ls.link_level,0)as link_level,lf.lid,lt.link_name,lt.type_id,lt.lang_id,case when ls.position is null then '9999999999' else ls.position end as lPos from web_links_structure ls 
                            INNER JOIN web_links_final lf on lf.lid=ls.lid INNER JOIN web_link_temp lt on lt.link_temp_id=lf.link_temp_id
                            LEFT JOIN (select count(ls_id)as tot,parent_ls_id from web_links_structure where STATUS='Active' and parent_ls_id is not null GROUP BY parent_ls_id) nls on nls.parent_ls_id=ls.ls_id where ls.`status`='Active' and ls.link_level is null and lt.lang_id=$lang_id and lt.content_type=$_REQUEST[nmnh_type_id] ORDER BY lt.link_name",1);
                                    ...
                                    foreach($rs[1] as $row){
                                        ...
                                        switch($row['type_id']){
                                            ...
                                            case 3:
                                                ...
                                                $imgName='images/expand.gif';
                                                $glypIcon='<i class="fa fa-laptop" aria-hidden="true"></i>';
                                            break;
        ...
        $(function(){
            ...
            $('.setSubLink').click(function(){
                ...
                $('.modal-container').OpenPop({
                    url:"setSub_links.php?lsid="+lsid+"&lid=" + lid +"&level=" + l_level + "&nmnh_type="+ nmnh_type+"&lang_id=<?= $_REQUEST['lang_id'] ?? '';?>&per_id=<?php echo "$_SESSION[per_id]&EncHid=$_SESSION[EncTok]";?>",
                });
            })
            ...
            $('.showTab').click(function(){
                ...
                if($('#tab_'+sId).is(':visible')){
                    //console.log('yes');
                    $('#img_'+sId).attr({'src':'images/expand.gif'});
                    $('#tab_'+sId).hide();
                }
                else{
                    //console.log('no');
                    $('#img_'+sId).attr({'src':'images/collapse.gif'});
                    $('#tab_'+sId).show();
