...
<h2 class="h5 content-header text-dark">Manage Tender Category</h2>

<div class="text-end mb-3">
    <button id="AddNew" cdata-frmT="1" class="btn btn-warning rounded-0 fw-bold px-3 py-1 add-btn"><i class="fas fa-plus-square me-1" aria-hidden="true"></i>Add New Category</button>
</div>  
    ...
    if($.inArray(frmType,['3','4','5','6']) !== -1){
        if(confirm('Do you really want to Proceed?')){
            ...
            $.ajax({
                ...
                dataType: "text",
                cache: false,
                url: "tender_cat_action.php?per_id=<?php echo $_SESSION['per_id']; ?>&EncHid=<?php echo $_SESSION['EncTok']; ?>",
                data: {
                    <?php echo $GLOBALS['csrf']['token']; ?>,
                    frmType: frmType,
                    t_cat_id: eleCtrl.data('t_cat_id')
                },
                beforeSend: function(){
                    $.fn.ajaxLoading();
                },
        ...
        $('.modal-container').OpenPop({
            url:"tender_cat.php?frmType="+frmType+
                "&t_cat_id="+eleCtrl.data('t_cat_id')+
...
$(function(){
    ...
    tabs.forEach(function(tabID){
        var table = $('#myTable'+tabID).DataTable({
            ...
            serverSide: false,
            ajax:{
                url:"AjaxFill/getTenderCat.php?EncHid=<?php echo $_SESSION['EncTok']; ?>",
                type:"POST",
                data:{
                    <?php echo $GLOBALS['csrf']['token']; ?>,
                    tabID: tabID,
