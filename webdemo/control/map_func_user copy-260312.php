          ...
          <div class="content-wrapper">
            <div class="box">
                ...
                <div class="box-body clearfix">
                    ...
                    <div class="panel-heading box-title">Sub Modules List</div>
                                    ...
                                    <input type="hidden" name="user_type_id" id="user_type_id" value="<?php echo $_REQUEST['user_type_id'];?>" />
                                     <div class="col-sm-8 col-sm-offset-2">                                             
                                      <table id="GridData" class="table table-bordered table-striped table-hover clearfix">
                                        ...
                                        $sNo='1';
                                        /*
                                        $rs1=simplefetch("select m.module_id,sm.sub_module_id,m.module_name,sm.sub_module_name,sm.fa_icon,sm.sub_module_page,sm.pos from web_st_sub_module sm
                                        INNER JOIN web_st_module m on m.module_id=sm.module_id where m.status='Active' and sm.status='Active'  group by m.module_id");
                                        */
                                        $rs1=simplefetch("select m.module_id,sm.sub_module_id,m.module_name,sm.sub_module_name,sm.fa_icon,sm.sub_module_page,sm.pos from web_st_sub_module sm
                                        INNER JOIN web_st_module m on m.module_id=sm.module_id where m.status='Active' and sm.status='Active' group by 
                                                    m.module_id,
                                                    sm.sub_module_id,
                                                    m.module_name,
                                                    sm.sub_module_name,
                                        ...
                                        if($rs1[0]>0){
                                            foreach($rs1[1] as $row1){                                        
                                                $rs=simplefetch("select sm.sub_module_id,m.module_name,sm.sub_module_name,sm.fa_icon,sm.sub_module_page,sm.pos,if(wfut.sub_module_id is not null,'checked=\"\"','')as flage from web_st_sub_module sm
                                                INNER JOIN web_st_module m on m.module_id=sm.module_id
                                                 LEFT JOIN(SELECT * from web_map_func_user_type where `status`='Active' and user_type_id=$_REQUEST[user_type_id]) wfut on sm.sub_module_id=wfut.sub_module_id 
                                                where m.status='Active' and sm.status='Active' and m.module_id=$row1[module_id] order by sm.pos",1);
                                                if($rs[0]>0){
                                                ?>
                                                <tr class="group"><td colspan="3"><?php echo $row1['module_name'];?></td></tr>
                                                <?php
                                                    foreach($rs[1] as $row){
                                                    ?>
                                                    <tr id="<?php echo $row['sub_module_id'];?>">
                                                        ...
                                                        <td class="text-center error-message">                                                            
                                                          <div><input type="checkbox" name="chk[]" id="chk_<?php echo $row['sub_module_id'];?>" value="<?php echo $row['sub_module_id'];?>" <?php echo $row['flage'];?> data-validate="<?php echo end($rs[1])==$row?"chk[]|checkbox|y|1|2|selmin=1|Please Select atleast One Checkbox to Proceed..":""?>">                                                            
                                                       </div></td>
                                                    </tr>
                                                    <?php                                
                                                    }
