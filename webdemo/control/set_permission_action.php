...
if (!empty($subStr)) {
    ...
    $success = update(
        "web_user_permission",
        ['status' => 'Deleted'],
        "sub_module_id not in($subStr) and user_id=$userId"
    );
...
if (!empty($SubFunc_id)) {
    ...
    foreach ($SubModuleArr as $val2) {
        ...
        $SubChk = getName(
            "web_user_permission",
            "per_id",
            "user_id=$userId and sub_module_id=$val2"
        );
...
if (!empty($perTypes)) {
    ...
    update(
        "web_user_permission_group",
        ['status' => 'Deleted'],
        "per_type_id not in($perTypes) and per_id=$SubChk"
    );
            ...
            foreach ($SubFunc_id as $val) {
                ...
                if (isset($_REQUEST['chk'.$val2.'_'.$val])) {
                    ...
                    $SubFuncChk = getName(
                        "web_user_permission_group",
                        "per_action_id",
                        "per_id=$SubChk and per_type_id=$val"
                    );
