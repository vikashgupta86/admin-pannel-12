...
<h4 class="modal-title">Add/ Modify Sub Module</h4>
            ...
            if(isset($_REQUEST['frmType']) && $_REQUEST['frmType'] == 2){
                $sub_module_id = (int)($_REQUEST['sub_module_id'] ?? 0);
                $rs = simplefetch("select sub_module_id,module_id,sub_module_name,sub_module_page,fa_icon,per_type_id from web_st_sub_module where status='Active' and sub_module_id=$sub_module_id");
            ...
            <form name="formNC" id="formNC" method="post" autocomplete="off" role="form" action="sub_module_action.php?per_id=<?= $_SESSION['per_id'] ?? ''; ?>&amp;EncHid=<?= $_SESSION['EncTok'] ?? ''; ?>">
            <input type="hidden" name="frmType" id="frmType" value="<?= $_REQUEST['frmType'] ?? ''; ?>" />
            <input type="hidden" name="sub_module_id" id="sub_module_id" value="<?= $_REQUEST['sub_module_id'] ?? ''; ?>" />
            <div class="modal-body">
                  <div class="box-body">
                    ...
                    <div class="mb-3">
                      <label for="sm_name" class="form-label">Sub Module Name</label>
                    ...
                    <div class="mb-3">
                      <label for="submodule_page" class="form-label">Page Name</label>
                    ...
                    <div class="mb-3">
                        <label for="per_types[]" class="form-label">Nature of Permissions</label>
                        <div class="checkbox">
                        <?php
                        $row_per_type_id = $row['per_type_id'] ?? '';
                        $rs2 = fetchcols("web_st_permission_type", "per_type_id,per_type_name,case when FIND_IN_SET(per_type_id,\"$row_per_type_id\") then 'checked=\"true\"' else '' end as chk", "1 = 1 order by pos");
                        if (isset($rs2[0]) && $rs2[0] > 0) {
