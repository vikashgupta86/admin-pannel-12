...
if(isset($_REQUEST['frmType'])&& $_REQUEST['frmType']!=3){
    ...
    $FielArr=array(
       ...
       // 'meta_tag'=>htmlspecialchars($_REQUEST['l_mdesc'],ENT_QUOTES),        
        ...
        /*'pub_date'=>!empty($_REQUEST['link_pubDate'])?$obj->todate($_REQUEST['link_pubDate']):'',
...
if(isset($_REQUEST['frmType'])){
    switch($_REQUEST['frmType']){
        case 1:#for the add
            $ChkLink= getNameQry("select t_temp_id from web_tender_temp where status='Active' and app_reject is null and corrigendum=1 and trim(lower(tender_name))=trim(lower('$_REQUEST[l_name]'))");
            if(!empty($ChkLink)){
                $result[2]=array(true,'Requested Corrigendum Name already exist!','alert-info');
                //goto ComeHere;                                                        
            }
            $FielArr=array_merge($FielArr,array(
                't_id'=>$_REQUEST['t_id'],
        ...
        case 2:
            $ChkLink= getNameQry("select t_temp_id from web_tender_temp where status='Active' and app_reject is null and corrigendum=1 and t_temp_id!=$_REQUEST[t_temp_id] and lower(trim(tender_name))=lower(trim('$_REQUEST[l_name]'))");
            if(!empty($ChkLink)){
                $result[2]=array(true,'Requested Corrigendum Name already exist!','alert-info');
                //goto ComeHere;                                                        
            }
            
            $ChkPub= simplefetch("SELECT tt.t_temp_id,tt.file_name from web_tender_temp tt INNER JOIN web_tender_corrigendum_final tf on tf.t_temp_id=tt.t_temp_id WHERE tt.`status`='Active' and tt.corrigendum=1 and tt.t_temp_id= ".$_REQUEST['t_temp_id']."");
            if($ChkPub[0]<=0){
                
                $tndrid = $_REQUEST['t_temp_id'];
                $success = update("web_tender_temp", $FielArr, "t_temp_id = $tndrid", 1 );
                ...
                $success = insert("web_tender_temp", $FielArr, 1);
               // $success=$obj->insert("web_tender_temp",$Fields,$Values);
            }
            
            
            if($success){#in case of revive
                $RevID=$obj->getName("web_tender_revive","t_revive_id","t_temp_id= ".$_REQUEST['t_temp_id']." and revive_status=1");
                if(!empty($RevID))
                {
                    ...
                    $success = simplefetchUA($updatetndrcat, "iisiii", [2, $_SESSION['userid'], $cDate, $RevID, 1, $tmpid]);

                    //$obj->update("web_tender_revive","revive_status|$$|revived_by|$$|revived_on","2|$$|$_SESSION[userid]|$$|$cDate","t_revive_id=$RevID and revive_status=1 and t_temp_id=$_REQUEST[t_temp_id]");
                }
            }
        break;
