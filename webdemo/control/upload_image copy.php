...
if (!in_array($fileExt, $allowedExts)) {
    echo json_encode(['error' => 'Extension not allowed. Use JPG, JPEG or PNG.']);
    exit;
}

$maxSize = 500 * 1024;
if ($file['size'] > $maxSize) {
    echo json_encode(['error' => 'Image too large. Max 500KB allowed.']);
    exit;
}
...
$finalName = 'RTF-IMG-' . bin2hex(random_bytes(8)) . '_' . time() . '.' . $fileExt;
