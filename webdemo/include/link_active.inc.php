    ...
    $LefBlock='col-md-3';
		...
		<?php if(!isset($_GET['lc_id'])){?>   
			...
			<?php
                ...
                <!-- ##SHOW DATA BASED ON DASHBOARD ENTRY STARTS## -->
                <?php
                $current_lid = (int)($_GET['lid'] ?? 0);
                $lang_id = (int)($_SESSION['lang'] ?? 1);
                
                /** * Optimized SQL:
                 * 1. Filters only Active status across all joined tables.
                 * 2. Joins web_links_structure carefully to avoid duplication.
                 */
                $sql = "SELECT 
                            p.prog_name, 
                            p.prog_name_hin, 
                            pd.attribute_name, 
                            pd.attribute_name_hin, 
                            pd.value, 
                            pd.value_type, 
                            y.years AS year_label,
                            ls.link_level
                        FROM web_dash_prog p
                        INNER JOIN web_dash_prog_data pd ON p.prog_id = pd.prog_id
                        INNER JOIN years y ON pd.year_id = y.year_id
                        INNER JOIN web_links_structure ls ON ls.lid = p.dash_sb_link_id
                        WHERE p.dash_sb_link_id = ? 
                          AND p.status = 'Active' 
                          AND pd.status = 'Active'
                          AND y.Status = 'Active' 
                          AND ls.status = 'Active'
                        GROUP BY y.years, pd.prog_data_id -- Prevents duplicate attribute rows
                        ORDER BY y.years DESC, p.prog_id ASC";
                
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("i", $current_lid);
                $stmt->execute();
                $allResults = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
                
                // Group results by year string
                $yearlyGroups = [];
                foreach ($allResults as $Dashrow) {
                    $yearlyGroups[$Dashrow['year_label']][] = $Dashrow;
                }
                
                // Slice to keep only the 3 most recent active years
                $yearlyGroups = array_slice($yearlyGroups, 0, 3, true);
                
                if (!empty($yearlyGroups)): 
                    foreach ($yearlyGroups as $displayYear => $rows): 
                ?>
                    <div class="dashboard-year-container" style="margin-bottom: 40px; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
                        <h3 style="text-align: center; background-color: #f8f9fa; color: #1f5692; padding: 15px; margin: 0; border-bottom: 2px solid #1f5692;">
                            <?= ($lang_id == 2) ? "वर्ष: " : "Year: " ?> <?= htmlspecialchars($displayYear) ?>
                        </h3>
                        
                        <table class="table table-bordered" style="width: 100%; margin-bottom: 0;">
                            <thead>
                                <tr style="background-color: #1f5692; color:#fff;">
                                    <th style="text-align:center; width: 8%;">S. No.</th>
                                    <th><?= ($lang_id == 2) ? 'विशेषता' : 'Attribute' ?></th>
                                    <th style="text-align:center; width: 25%;"><?= ($lang_id == 2) ? 'प्रकार' : 'Value Type' ?></th>
                                    <th style="text-align:center; width: 20%;"><?= ($lang_id == 2) ? 'मान' : 'Value' ?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php 
                                ...
                                foreach ($rows as $item): 
                                    $attr = ($lang_id == 2 && !empty($item['attribute_name_hin'])) 
                                            ? $item['attribute_name_hin'] 
                                            : $item['attribute_name'];
                                ?>
                                <tr style="background-color: #fff; color: #333;">
                                    <td style="text-align: center;"><?= $sn++ ?></td>
                                    <td><?= html_entity_decode($attr ?? '') ?></td>
                                    <td style="text-align: center;"><?= html_entity_decode($item['value_type'] ?? '') ?></td>
                                    <td style="text-align: center; font-weight: bold; color: #000;"><?= html_entity_decode($item['value'] ?? '') ?></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                ...
                <!--
                    <div class="alert alert-warning" style="text-align: center;">
                        <?= ($lang_id == 2) ? 'कोई सक्रिय डेटा उपलब्ध नहीं है' : 'No active dashboard data available.' ?>
                    </div>
                --><?php 
                endif; 
                ?>
                ...
                <!-- ##SHOW DATA BASED ON DASHBOARD ENTRY ENDS## -->
...
//                                if($rs3[0] > 0){
                    ...
                    //echo('123');
                        ...
                        foreach($rs3[1] as $row3){
                            ...
                            $showMid=($rs3[0]==$lPos && $lPos++%3==1)?" col-md-offset-4":'';                                                                                    
                    ?>                    
                        <div class="col-md-4 <?php echo $bookClass.$showMid;?>">
                            <div  style="background-color: #fff;box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);background-clip: border-box;border: 2px solid rgba(0, 0, 0, .125);border-radius: 15px; min-height:350px; margin-bottom:10px;">
                            <!--<img src="WriteReadData/IC1425/<?php echo $row3['icon_name']?>" height="150px" />-->
                                <p class="booksmain <?php echo !empty($sPopClass)?'CopyIcon '.$sPopClass:'';?>" <?php echo !empty($sPopClass)?"data-lid='$row3[lid]'":'';?>>
                                    ...
                                    if(empty($bookClass)){
                                    ?>
                                        <div class="imghover-inner text-center" ><a <?php echo $row3['l_target'];?> title="<?php echo CleanW3cWarn($row3['title'])?>" href="<?php echo empty($bookClass)?"$newlinkfile?lang=$_SESSION[lang]&amp;level=$row3[link_level]&amp;ls_id=$row3[ls_id]&amp;lid=$row3[lid]{$dUrl}":"javascript:void(0);"?>" style="font-size: 14px;"><img src="WriteReadData/IC1425/<?php echo $row3['icon_name']?>" height="220px" width="250px" /><br/><br/><?php echo CleanW3cWarn($row3['link_name'])?></a></div>                                
                                    <?php    
                                    }
                                    else{
                                    ?>
                                    <img src="WriteReadData/IC1425/<?php echo $row3['icon_name']?>" height="220px" width="280px" /><?php echo CleanW3cWarn($row3['link_name']);                                
                                    }
                                    ?>
                                </p>
                                <div style="padding-left:17px; color:#614b4b"><?php echo _html_entity_decode($row3['author_name'] ?? '');?><?php echo empty($bookClass)?(!empty($row3['pub_date'])?"&nbsp;|&nbsp;$row3[pub_date]":""):(!empty($row3['link_bdesc'])?"<br>$row3[link_bdesc]":"");?></div>
                            </div>
                        </div>
                    <?php                        
                        echo ($lPos++%3==0)?"</div><div class='row'>":'';
...
$(document).ready(function() {
	...
	/*$(".fancybox").fancybox({	   
		...
		helpers	: {
            ...
            <div class="row">
                <div class="text-right"><button style="background:#004696" type="submit" id="l_feedback" class="btn btn-primary" data-hrf='l_feedback' href='feedback.php?lang=<?= $_SESSION['lang']; ?>&level=<?= $row['level']; ?>&ls_id=<?= $row['ls_id']; ?>&lid=<?= $row['lid']; ?>"'>HGHG<a style="color: #fff;" href="<?php echo "feedback.php?lang=$_SESSION[lang]&level=$row[level]&ls_id=$row[ls_id]&lid=$row[lid]";?>">Post Comments</a></button></div>
            </div>
            <?php }?>     
        </div>
    <?php
