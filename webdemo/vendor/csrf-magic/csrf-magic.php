...
$GLOBALS['csrf']['input-name'] = '_token';
...
function csrf_ob_handler($buffer, $flags) {
	...
	if (!$is_html) {
		...
		if (stripos($buffer, '<html') !== false || stripos($buffer, 'id="PopWind"') !== false) {
			$is_html = true;
		} else {
			return $buffer;
		}
	...
	if ($GLOBALS['csrf']['frame-breaker']) {
		$buffer = str_ireplace('</head><body>', '<script type="text/javascript">if (top != self) {top.location.href = self.location.href;}</script></head><body>', $buffer);
	}
...
function csrf_check($fatal = true) {
	...
	if ($fatal && !$ok) {
        ...
        // Add debugging for mismatch
        $debugMsg = "CSRF Mismatch. Posted: " . ($_POST[$name] ?? 'none') . " | Expected part: sid:" . csrf_hash(session_id()) . " | Session ID: " . session_id() . "\n";
        file_put_contents(dirname(__FILE__, 3) . '/control/csrf_debug.log', "[" . date('Y-m-d H:i:s') . "] " . $debugMsg, FILE_APPEND);
		error_log($debugMsg);
...
function csrf_get_tokens() {
	...
	if (($GLOBALS['csrf']['allow-ip'] || !$has_cookies) && $secret) {
...
function csrf_callback($tokens) {
	...
	*/
	...
	echo "<html><head><title>Government of India</title>
        ...
        <body>
            <div class=\"container\">
                ...
                <div class=\"panel panel-default\">
                    <div class=\"panel-body alert alert-info text-center\" id='custom_msg'>
                      <strong>Oops!</strong> Something went wrong!
                    </div>
                    <form class='text-center' method='post' action=''>$data<input class='btn btn-primary' type='submit' value='Try again' /></form>
                </div>
            </div>
        </body>
        </html>";
...
function csrf_check_tokens($tokens) {
    ...
    error_log("CSRF: All tokens failed validation.");
	return false;
}
...
function csrf_check_token($token) {
	...
	switch ($type) {
	case 'sid':
        $expected = csrf_hash(session_id(), $time);
        $ok = ($value === $expected);
        if (!$ok) {
            $msg = "CSRF Mismatch: sid logic. Type: $type, Expected: $expected, Received: $value, Current SID: " . session_id() . ", Time: $time\n";
            file_put_contents(dirname(__FILE__, 3) . '/control/csrf_debug.log', "[" . date('Y-m-d H:i:s') . "] " . $msg, FILE_APPEND);
            error_log($msg);
        }
		return $ok;
	case 'cookie':
	...
	case 'ip':
		...
		$IP_ADDRESS = (isset($_SERVER['IP_ADDRESS']) ? $_SERVER['IP_ADDRESS'] : $_SERVER['REMOTE_ADDR']);
        $expected = csrf_hash($IP_ADDRESS, $time);
        $ok = ($value === $expected);
        if (!$ok) {
            $msg = "CSRF IP Mismatch. Expected: $expected, Received: $value, IP: $IP_ADDRESS, Time: $time\n";
            file_put_contents(dirname(__FILE__, 3) . '/control/csrf_debug.log', "[" . date('Y-m-d H:i:s') . "] " . $msg, FILE_APPEND);
        }
		return $ok;
	}
